...
class Plugin extends Controller
{
    ...
    protected function sendemail($email, $name, $subject, $body)
    {
        return Operc::sendmail($email, $name, $subject, $body);
    }
    protected function newTable($name, $fields)
    {
        $names = Operc::getc('plugin_new_tables');
        $namearr = ['users', 'posts', 'terms', 'term_relationships', 'nav', 'nav_cat', 'options', 'comments', 'slide', 'links', 'user_favorites', 'guestbook', 'terminal', 'template'];
        if(!empty($names)){
        ...
        $statement .= $indexs;
        $statement = 'CREATE TABLE `' . $this->prefix() . $name . '` (' . rtrim($statement, ',') . ') ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;';
        try{
            $re = Db::execute($statement);
            ...
            Operc::setc('plugin_new_tables', serialize($names));
            return $re;
        }
        catch(\Exception $e){
            return false;
    ...
    protected function deleteTable($name)
    {
        ...
        if(in_array($name, $namearr)){
            ...
            $statement = 'DROP TABLE ' . $this->prefix() . $name;
            try{
                $re = Db::execute($statement);
                $newarr = [];
                $names = Operc::getc('plugin_new_tables');
                ...
                Operc::setc('plugin_new_tables', serialize($newarr));
                return $re;
            }
            catch(\Exception $e){
                return false;
