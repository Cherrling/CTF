...
class Index extends Common
{
    ...
    public function write()
    {
        ...
        $this->assign('fenlei', $this->getfenlei());
        $this->assign('pushpull', Operc::aut() ? 1 : 0);
    ...
    public function articles()
    {
        $this->checkUser();
        $this->checkPermissions(6);
        $zongjilu = Cache::get('admin_articles_zongjilu_'.$this->lang);
        ...
        if($zongjilu === false){
            $zongjilu = $data->total();
            Cache::set('admin_articles_zongjilu_'.$this->lang,$zongjilu,3600);
        }
        $this->assign('data', $data);
        $this->assign('backstageMenu', 'neirong');
    ...
    public function comments()
    {
        $this->checkUser();
        $this->checkPermissions(5);
        $zongjilu = Cache::get('admin_comments_zongjilu_'.$this->lang);
        $data = Db::view('comments','id,post_id,createtime,content,status')
            ->view('users','user_login,user_nicename,user_email,avatar','users.id=comments.uid')
            ->order('comments.createtime desc')
            ->paginate(10, $zongjilu);
        if($zongjilu === false){
            $zongjilu = $data->total();
            Cache::set('admin_comments_zongjilu_'.$this->lang,$zongjilu,3600);
        }
        $this->assign('data', $data);
        $this->assign('backstageMenu', 'neirong');
    ...
    public function messages()
    {
        $this->checkUser();
        $this->checkPermissions(5);
        $zongjilu = Cache::get('admin_messages_zongjilu_'.$this->lang);
        $data = Db::name('guestbook')->order('createtime desc')->paginate(10, $zongjilu);
        if($zongjilu === false){
            $zongjilu = $data->total();
            Cache::set('admin_messages_zongjilu_'.$this->lang,$zongjilu,3600);
        }
        $this->assign('data', $data);
        $this->assign('backstageMenu', 'neirong');
    ...
    public function search()
    {
        ...
        {
            $qkey = $key = Request::instance()->get('key');
            if($this->permissions < 6){
                $tmpkey = str_replace('：', ':', $key);
                if(false !== $wz = strpos($tmpkey, ':')){
                    $leftkey = trim(substr($tmpkey, 0, $wz));
                    if(strtolower($leftkey) == 'author' || $leftkey == '作者'){
                        $author = trim(substr($tmpkey, $wz + 1));
                        $key = '';
                    }
                }
            }
        ...
        if($fenlei != 0)
        {
            if($this->permissions < 6)
            {
                $data = Db::view('posts','id,post_date,post_title,post_status,comment_count,thumbnail,post_hits,istop,recommended,status,fujian')
                    ->view('users','user_login','users.id=posts.post_author')
                    ...
                    ->where('users.user_login|users.user_nicename','like','%'.$author.'%')
                    ->order('post_date desc')
                    ->paginate(10,false,[
                        'query' => [
    ...
    public function rewrite()
    {
        ...
        if(Request::instance()->has('postId','post') && $this->verification())
        {
            ...
            Hook::add('rewrite',$this->plugins);
            $params = [
                'title' => $biaoti,
                'content' => $neirong,
                'summary' => ltrim($zhaiyao)
            ];
            Hook::listen('rewrite',$params,$this->ccc);
            $biaoti = $params['title'];
            $neirong = $params['content'];
            $zhaiyao = $params['summary'];
            $data = ['post_keywords' => htmlspecialchars($guanjianci), 'post_source' => str_replace('"', '\'', $this->filterJs(Request::instance()->post('laiyuan'))), 'zuozhe' => trim(Request::instance()->post('zuozhe')), 'bianji' => trim(Request::instance()->post('bianji')), 'post_content' => $neirong, 'post_title' => htmlspecialchars($biaoti), 'post_excerpt' => htmlspecialchars($zhaiyao), 'comment_status' => Request::instance()->post('pinglun'), 'post_modified' => date("Y-m-d H:i:s"), 'post_type' => Request::instance()->post('xingshi'), 'thumbnail' => $suolvetu, 'istop' => Request::instance()->post('zhiding'), 'recommended' => Request::instance()->post('tuijian'), 'fujian' => Request::instance()->post('fujian'), 'fujianurl' => $this->filterUrl(Request::instance()->post('fujianurl'))];
            Db::name('posts')
    ...
    public function newpage()
    {
        ...
        if(Request::instance()->has('biaoti','post') && $this->verification())
        {
            ...
            if(in_array($template,$dir))
            {
                ...
                $data = ['post_author' => Session::get($this->session_prefix.'user_id'), 'post_keywords' => htmlspecialchars($guanjianci), 'zuozhe' => trim(Request::instance()->post('zuozhe')), 'bianji' => trim(Request::instance()->post('bianji')), 'post_date' => Request::instance()->post('fabushijian'), 'post_content' => $neirong, 'post_title' => htmlspecialchars(Request::instance()->post('biaoti')), 'post_excerpt' => htmlspecialchars(Request::instance()->post('zhaiyao')), 'post_modified' => Request::instance()->post('fabushijian'), 'post_type' => 1, 'thumbnail' => $suolvetu, 'template' => $template, 'fujian' => Request::instance()->post('fujian'), 'fujianurl' => str_replace('，', ',', Request::instance()->post('fujianurl'))];
                $id = Db::name('posts')->insertGetId($data);
    ...
    public function allpage()
    {
        $this->checkUser();
        $this->checkPermissions(3);
        $zongjilu = Cache::get('admin_allpage_zongjilu_'.$this->lang);
        ...
        if($zongjilu === false){
            $zongjilu = $data->total();
            Cache::set('admin_allpage_zongjilu_'.$this->lang,$zongjilu,3600);
        }
        $this->assign('data', $data);
        $this->assign('backstageMenu', 'yemian');
    ...
    public function addLinks()
    {
        ...
        if(Request::instance()->isPost() && $this->verification())
        {
            ...
            $msg = [
                ...
                'dizhi.url' => Lang::get('Friendship link address is invalid')
            ];
            $data = [
    ...
    public function general()
    {
        $this->checkUser();
        $this->checkPermissions(3);
        $zongjilu = Cache::get('admin_general_zongjilu_'.$this->lang);
        $data = Db::name('users')->where('user_type',7)->order('last_login_time desc')->paginate(10, $zongjilu);
        if($zongjilu === false){
            $zongjilu = $data->total();
            Cache::set('admin_general_zongjilu_'.$this->lang,$zongjilu,3600);
        }
        $pages = $data->render();
    ...
    public function web()
    {
        ...
        if(Request::instance()->has('title','post') && $this->verification())
        {
            ...
            Db::name('options')
                ->where('option_name', 'write')
                ->update(['option_value' => Request::instance()->post('write')]);
            Cache::clear();
        }
        ...
        foreach($siteInfo as $key => $val){
            ...
            else if($val['option_name'] == 'write')
            {
                if(empty($val['option_value']))
                {
                    $data[$val['option_name']] = '';
                }
                else{
    ...
    public function themes()
    {
        ...
        $obt = Operc::getc('catfish_cmscom_obth');
        if(empty($obt)){
            $obt = [];
        }
        else{
            ...
            if(!is_array($obt)){
                $obt = explode(',', $obt);
                $obt = array_map(function($v){return trim($v);}, $obt);
            }
        }
        $themes = [];
        $domain = $this->host();
        ...
        foreach($dir as $key => $val)
        {
            ...
            if($tmpdir != 'common' && $tmpdir != 'data' && strtolower(substr($tmpdir,0,5)) != 'cblog')
            {
                ...
                if(is_file($path))
                {
                    ...
                    if(imagesx($img) == 414 && imagesy($img) == 315 && !in_array($tmpdir, $obt)){
                        continue;
                    }
    ...
    public function dbbackup()
    {
        $this->checkUser();
        $this->checkPermissions(3);
        if(Request::instance()->has('dbbackup','post') && $this->verification())
        {
            ...
            $bkstr = '-- 鲶鱼CMS数据库备份' . PHP_EOL . '-- 生成日期：' . date('Y-m-d H: i: s') . PHP_EOL . '-- Table prefix: ' . $dbPrefix . PHP_EOL . '-- Backup tables: ' . implode(',', $onlynms) . PHP_EOL . $bkstr;
            $bkpath = date('Ymd');
    ...
    public function uploadrestore()
    {
        ...
        $this->checkPermissions(3);
        $prompt = '';
        if(Request::instance()->has('uploadrestore','post') && $this->verification())
        {
            ini_set('max_execution_time', 0);
            ini_set('memory_limit', -1);
            $file = request()->file('file');
            if($file->checkExt('cbk') === true){
    ...
    public function plugin()
    {
        ...
        $obt = Operc::getc('catfish_cmscom_obt');
        if(empty($obt)){
            $obt = [];
        }
        else{
        ...
        foreach($dir as $key => $val)
        {
            ...
            if(preg_match("/(插件网址|插件網址|Plugin URI|Plugin URL)\s*(：|:)(.*)/i", $pluginStr ,$matches))
            {
                ...
                if(strpos($pluginUri, 'Catfish-cms') !== false && !in_array($pluginName, $obt)){
                    continue;
                }
            }
            $quanxian = 3;
    ...
    private function view($template = '')
    {
        ...
        $ns = Operc::aut() ? Operc::bdc('IHN0eWxlPSJkaXNwbGF5OiBub25lOyI=') : '';
        $this->assign('catfish', '<a href="http://www.'.$version['official'].'/" target="_blank"'.base64_decode('IGlkPSJjYXRmaXNoIg==').$ensure.$ns.'>'.$version['name'].'&nbsp;'.$version['description'].'&nbsp;'.$version['number'].'</a>&nbsp;&nbsp;');
    ...
    public function smtpsettings()
    {
        $this->checkUser();
        $this->checkPermissions(1);
        if(Request::instance()->has('host','post') && $this->verification())
        {
            $rule = [
                'host' => 'require',
                'port' => 'require',
            ...
            Operc::setc('emailsettings', $estis);
        }
        $data = Operc::getc('emailsettings');
        if(empty($data)){
            $data = [
                'host' => '',
                'port' => 25,
    ...
    public function sendtestmail()
    {
        if($this->verification())
        {
            $estis = unserialize(Operc::getc('emailsettings'));
            if(Operc::sendmail($estis['user'], '', Lang::get('Test mail'), Lang::get('This is a test email'))){
                echo 'ok';
            }
            else{
    ...
    {
        if(Request::instance()->has('act','post') && Request::instance()->post('act') == 'push' && $this->verification()){
            $time = time();
    ...
    {
        if(Request::instance()->has('act','post') && Request::instance()->post('act') == 'pull' && $this->verification()){
    ...
    {
        if(Request::instance()->has('act','post') && Request::instance()->post('act') == 'savetemplate' && $this->verification()){
    ...
    {
        if(Request::instance()->has('act','post') && Request::instance()->post('act') == 'loadtemplate' && $this->verification()){
    ...
    {
        if(Request::instance()->has('act','post') && Request::instance()->post('act') == 'fujianupload' && $this->verification()){
            $result = $this->checkPermissions(6, true);
            if(is_array($result)){
                return json($result);
            }
            $file = request()->file('file');
    ...
    {
        if(Request::instance()->has('act','post') && Request::instance()->post('act') == 'shanchufujian' && $this->verification()){
    ...
    public function uploadtheme()
    {
        ...
        if($this->verification()){
            ...
            $file->validate($validate);
            $tmpdir = ROOT_PATH . 'runtime' . DS . 'themes';
            $this->delFolder($tmpdir);
            $info = $file->move($tmpdir, '');
            if($info){
                $fileName = $info->getSaveName();
                $tmpFile = $tmpdir . DS . $fileName;
                if($fileName != 'common.zip' && is_file($tmpFile) && substr(strtolower($fileName), 0, 5) != 'cblog'){
                    try{
                        $zip = new \ZipArchive();
                        if($zip->open($tmpFile, \ZipArchive::OVERWRITE || \ZIPARCHIVE::CREATE) === true){
                            $zip->extractTo(ROOT_PATH . 'public');
                            $zip->close();
                            if(false !== $dw = strpos($fileName, '.')){
                                $fileName = substr($fileName, 0, $dw);
                            }
                            $this->scanfiles(ROOT_PATH . 'public' . DS . $fileName);
                            $this->chkphp(ROOT_PATH . 'public');
                            echo 'ok';
                        }
                        else{
    ...
    public function deltheme()
    {
        $this->checkPermissions(3);
        if($this->verification()){
            $file = Request::instance()->post('name');
            if(strpos($file, '.') === false){
                $tmpdir = ROOT_PATH . 'public' . DS . $file;
                $this->deleteFolder($tmpdir);
                echo 'ok';
            }
            else{
                echo Lang::get('Your operation is illegal');
            }
        }
        exit();
    }
                ...
                if(is_file($tmpFile)){
                    try{
                        ...
                        if($zip->open($tmpFile, \ZipArchive::OVERWRITE || \ZIPARCHIVE::CREATE) === true){
                            ...
                            $this->scanfiles(APP_PATH . 'plugins' . DS . $fileName);
                            $this->chkphp(APP_PATH . 'plugins');
                            echo 'ok';
                        }
                        else{
                            echo Lang::get('The uploaded zip file is not available');
    ...
    public function delplugin()
    {
        ...
        if($this->verification()){
            $file = Request::instance()->post('name');
            if(strpos($file, '.') === false){
                $tmpdir = APP_PATH . 'plugins' . DS . $file;
                $this->deleteFolder($tmpdir);
                echo 'ok';
            }
            else{
