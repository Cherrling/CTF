...
class Index extends Controller
{
    ...
    public function register()
    {
        ...
        {
            $rule = [
                'user' => 'require|regex:^[A-Za-z][A-Za-z0-9_\-]+$',
                'pwd' => 'require',
                'repeat' => 'require',
                'email' => 'require|email'
            ];
            $msg = [
                'user.require' => Lang::get('The user name must be filled in'),
                'user.regex' => Lang::get('Username can only use letters, numbers and connecting lines, and start with a letter'),
                'pwd.require' => Lang::get('Password must be filled in'),
    ...
    public function zhuce()
    {
        ...
        {
            return Lang::get('Username can only use letters, numbers and connecting lines, and start with a letter');
        }
        if($pwd == '')
        {
            return Lang::get('Password must be filled in');
    ...
    public function retpwd()
    {
        if(Request::instance()->has('user','post'))
        {
            ...
            Operc::sendmail($data['email'], $data['user'], Lang::get('Retrieve password'), Lang::get('This is your new password, please change your password immediately after login.'). '<br><br>'.Lang::get('Password').': '.$newpwd);
